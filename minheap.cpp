/*
 ...
 Chapter 11, Project Five (Minheap)
 Zain Umerani, COEN 70 T/R, 5:10 - 7:00 PM
 
 Professor Vu Hien
 
 
 This is the implementation for a minheap using a vector. I spoke to the professor and he said implementing
 it using a vector and without the 'order' array for priority is fine. This implementation also
 covers the case where we insert a series of same elements. Works 100%.
...
Data Heap<Data>::pop() {
    ...
    while( !isLeaf(i) && ( data[i] > data[ minChild(i) ] ) ) {
        swap( data[i] , data[ minChild(i) ] );
        i = minChild(i);
    }
